...
public class GUITutorialMulti : FiaGUILayer
{
	public GUITutorialMulti()
	{
	...
	public override void DoInit()
	{
		...
		this.bg_ = instance.CreateByWindowSpace(num, array, fiaTexture, 3, GUIFontCalculator.DEFAULT_GAP);
		this.panelManager_.RegistGUIInterface(this.bg_);
		this.back_ = new GUIPanelEx3PartVert(0, new float[] { 105f, 86f, 697f, 328f, 260f, 971f, 1022f }, fiaTexture, 2, GUIFontCalculator.DEFAULT_GAP, 3f);
		this.back_.RegistPanelManager(this.panelManager_);
		this.img_ = new GUIPanelEx[1];
		for (int i = 0; i < 1; i++)
		{
			this.img_[i] = GUIPanelFactory.Instance.CreateByWindowSpace(0, this.PANEL_INFO[i], fiaTexture, 1, GUIFontCalculator.DEFAULT_GAP);
			this.panelManager_.RegistGUIInterface(this.img_[i]);
		}
		this.buttons_ = new GUIButton[3];
		for (int j = 0; j < 3; j++)
		{
			this.buttons_[j] = new GUIButton(0, this.BUTTON_INFO[j], fiaTexture, 1, new Vector3(129f, 0f, 1f));
		...
		float[] array2 = new float[] { 120f, 222f, 682f, 310f, 2f, 632f, 564f, 720f };
		this.contents_ = new GUIUVScrollImage(0, array2, fiaTexture, 1, null);
	...
	protected override void AfterPanelUpdate()
	{
		...
		if (this.mouseManager_.IsSelected())
		{
			...
			else if (selectedKey == 2)
			{
				base.gameObject.SetActiveRecursively(false);
				base.SendMessage(1, ((MonoBehaviourMessage1Param<GameStageCommand>)MonoBehaviourMessageFactory.Instance.GetMessage(MonoBehaviourMessageType.GAMESTAGE_COMMAND)).Initialize(GameStageCommand.RESUME));
				KartOptions.Instance.SetQuestFlag(KartOptions.QuestFlag.TUTORIAL_MULTI, true);
				KartOptions.Instance.SaveRegistry();
				StageController.Instance.InputAutority = this.inputAuthorityBackUp_;
				StageController.Instance.ChangeStage(StageType.WIFI);
			}
	...
	public override void ReceiveMessage(int sender, MonoBehaviourMessage msg)
	{
		...
		if (msg.type_ == MonoBehaviourMessageType.SHOW_TUTORIAL_MULTI && !base.gameObject.active)
		{
	...
	private float[][] PANEL_INFO = new float[][] { new float[] { 197f, 101f, 566f, 527f, 996f, 640f } };
